...
export function BottomNav({onNavigate}: BottomNavProps) {
  return (
    <View style={styles.bottomNav}>
      <NavItem label="Trang chủ" icon="⌂" active onPress={() => onNavigate('Home')} />
      <NavItem label="Lịch sử" icon="◷" onPress={() => onNavigate('History')} />
      <Pressable onPress={() => onNavigate('Alerts')} style={styles.addButton}>
        <Text style={styles.addButtonText}>＋</Text>
      </Pressable>
      <NavItem label="Thiết bị" icon="▤" onPress={() => onNavigate('Devices')} />
      <NavItem label="Cài đặt" icon="⚙" onPress={() => onNavigate('Settings')} />
    </View>
  );
}
